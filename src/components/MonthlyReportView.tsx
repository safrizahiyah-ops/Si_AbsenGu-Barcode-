...
export const MonthlyReportView: React.FC = () => {
  ...
  const monthName = INDONESIAN_MONTHS[selectedMonth];
  ...
  const handleDownloadPdf = () => {
    generateMonthlyReportPdf(monthlyData, monthName, selectedYear, settings);
    showToast('Laporan Rekap Bulanan berhasil diunduh dalam format PDF.', 'success');
  };
  ...
  const handleCopyGoogleDocs = async () => {
    ...
    const fullHtml = `
      <div style="font-family: Arial, sans-serif; color: #000;">
        <div style="text-align: center; margin-bottom: 20px;">
          ...
          <h3 style="margin: 0; font-size: 15px; font-weight: bold;">REKAPITULASI ABSENSI GURU BULANAN</h3>
          <p style="margin: 5px 0 15px 0; font-size: 12px;">Bulan: ${monthName} ${selectedYear}</p>
        </div>
        ...
        <table style="width: 100%; border-collapse: collapse; font-size: 12px; margin-bottom: 30px;">
          <thead>
            <tr style="background-color: #f2f2f2;">
              ...
              <th style="border: 1px solid #000; padding: 8px; text-align: center;">Dinas</th>
              <th style="border: 1px solid #000; padding: 8px; text-align: center;">Tidak Hadir</th>
              <th style="border: 1px solid #000; padding: 8px; text-align: center;">Persentase Kehadiran</th>
            </tr>
          </thead>
          <tbody>
            ${rowsHtml}
        ...
        <table style="width: 100%; font-size: 12px; margin-top: 30px;">
          <tr>
            ...
            <td style="width: 50%; vertical-align: top; text-align: right;">
              <p style="margin: 0;">Darul Mahfudz, Akhir ${monthName} ${selectedYear}</p>
    ...
    const plain = `${settings.schoolName}\nREKAPITULASI BULANAN\nBulan: ${monthName} ${selectedYear}\n\n` +
      monthlyData.map((r, i) => `${i + 1}. ${r.teacherName}: Hadir=${r.hadir}, Terlambat=${r.terlambat}, Izin=${r.izin}, Sakit=${r.sakit}, Dinas=${r.dinas}, Alpha=${r.tidakHadir} (${r.percentage.toFixed(1)}%)`).join('\n');

    const success = await copyToGoogleDocsHtml(fullHtml, plain);
    if (success) {
      setIsCopied(true);
      showToast('Tabel Rekap Bulanan berhasil disalin ke clipboard.', 'success');
  ...
  const handleExportCsv = () => {
    ...
    downloadCsv(`Rekap-Absensi-Bulanan-${monthName}-${selectedYear}.csv`, headers, rows);
  };
      ...
      <div className="no-print bg-white rounded-2xl border border-slate-200 p-5 shadow-xs flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Laporan rekapitulasi kehadiran seluruh guru per bulan untuk evaluasi madrasah
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <div className="flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-200 text-xs">
            ...
            <span className="font-bold text-slate-600 ml-1">Tahun:</span>
            <select
              value={selectedYear}
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-md p-6 sm:p-8 printable-report">
        <div className="text-center pb-4 border-b-2 border-slate-900 mb-6">
          ...
          <h3 className="text-base font-extrabold text-slate-900 mt-3 underline">
            REKAPITULASI ABSENSI GURU BULANAN
          </h3>
          ...
          <table className="w-full text-left text-xs sm:text-sm border-collapse border border-slate-900">
            <thead className="bg-slate-100 text-slate-900 font-bold border-b border-slate-900">
              <tr>
                ...
                <th className="border border-slate-900 py-2 px-2 text-center">Dinas</th>
        ...
        <div className="mt-12 pt-6 grid grid-cols-2 gap-8 text-xs sm:text-sm">
          ...
          <div className="text-right">
            <p className="font-medium">
              Darul Mahfudz, Akhir {monthName} {selectedYear}
            </p>
